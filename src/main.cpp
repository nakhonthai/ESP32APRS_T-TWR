...
bool EVENT_TX_POSITION = 0;
unsigned char SB_SPEED = 0, SB_SPEED_OLD = 0;
int16_t SB_HEADING = 0;
uint16_t tx_interval = 0;             // How often we transmit, in seconds
unsigned int tx_counter;              // Incremented every second
int16_t last_heading, heading_change; // Keep track of corner pegging
uint16_t sta_interval = 15;

void smartbeacon(void)
{
  // Adaptive beacon rate
  if (SB_SPEED >= config.sta_hspeed)
  {
    ...
    tx_interval = (uint16_t)config.sta_maxinterval;
  }
  else
  {
    if (SB_SPEED <= config.sta_lspeed)
    {
      // Speed is below lower limit - use minimum rate
      tx_interval = (uint16_t)config.sta_slowinterval;
    }
    else
    {
      // In between, do SmartBeaconing calculations
      tx_interval = (sta_interval * config.sta_hspeed) / SB_SPEED;
      if (tx_interval < 5)
        tx_interval = 5;
      if (tx_interval > config.sta_slowinterval)
        tx_interval = (uint16_t)config.sta_mininterval;
      ...
      if (heading_change > config.sta_minangle)
      {
        // Make sure last heading is updated during turns to avoid immedate xmit
        if (tx_counter > config.sta_mininterval)
        {
          EVENT_TX_POSITION = 1;
        }
        else
        {
          ...
          tx_interval = config.sta_mininterval;
        }
      }
    }
  }
...
void defaultConfig()
{
  ...
  sprintf(config.aprs_mycall, "HS5TQA");
  config.aprs_ssid = 7;
  sprintf(config.aprs_host, "aprs.dprns.com");
  config.aprs_port = 14580;
  sprintf(config.aprs_passcode, "24484");
  ...
  config.mygps = true;
  config.sta_smartbeacon = false;
  config.sta_compress = false;
  config.sta_altitude = true;
  config.sta_speed = true;
  config.sta_hspeed = 40;
  config.sta_lspeed = 5;
  config.sta_maxinterval = 30;
  config.sta_mininterval = 5;
  config.sta_minangle = 25;
  config.sta_slowinterval = 600;
  saveEEPROM();
}
...
String myBeacon(String Path)
{
  ...
  if (config.sta_smartbeacon)
  {
    if (SB_SPEED < config.sta_lspeed)
    {
      aprs_table = config.sta_symstop[0];
      aprs_symbol = config.sta_symstop[1];
      SB_SPEED = 0;
    }
    else
    {
      aprs_table = config.sta_symmove[0];
      aprs_symbol = config.sta_symmove[1];
    }
  }
  else
  {
    tx_interval = config.sta_slowinterval = 60;
    aprs_table = config.mysymbol[0];
    aprs_symbol = config.mysymbol[1];
  }

  char object[10];
  memset(object, 0, 10);
  memcpy(object, config.tnc_item, strlen(config.tnc_item));
  object[9] = 0;
  ...
  if (gps.location.isValid())
  {
    ...
    if ((spdKnot <= 0.1) && (config.sta_altitude))
    {
...
boolean APRSConnect()
{
  ...
  if (con <= 0)
  {
    ...
    {
      uint16_t passcode = aprsParse.passCode(config.tnc_item);
      login = "user " + String(config.tnc_item) + " pass " + String(passcode, DEC) + " vers T-TWR+ V" + String(VERSION) + " filter " + String(config.aprs_filter);
    }
    else
    {
      if (config.aprs_ssid == 0)
...
//                     aprs_symbol = config.sta_symmove[1];
//                 }
//                 if (config.sta_speed && gps.speed.isValid())
//                 {
//                     if (config.sta_altitude)
...
//             else
//             {
//                 tx_interval = config.sta_slowinterval = 600;
...
//         else
//         {
//             tx_interval = config.sta_slowinterval;
...
void setup()
{
  ...
  //   if(!SerialBT.begin("DRHotspot")){
  //     log_d("An error occurred initializing Bluetooth");
  //   }else{
  ...
  // // delay(1);
  // // targetVol = PMU.getDC1Voltage();
  // // Serial.printf("BLDO1  :%s   Voltage:%u mV \n", PMU.isEnableDC1() ? "ENABLE" : "DISABLE", targetVol);
  // // if (targetVol != vol)
  ...
  // Serial.printf("MIC_VCC  :%s   Voltage:%u mV \n", PMU.isEnableBLDO1() ? "ENABLE" : "DISABLE", targetVol);
  // if (targetVol != vol)
  //   Serial.println(">>> FAILED!");
...
//     }
  ...
  String send_fix_location()
  {
    ...
    if (strlen(config.tnc_item) >= 3)
    {
      sprintf(loc, ")%s!%02d%02d.%02dN%c%03d%02d.%02dE%c", config.tnc_item, lat_dd, lat_mm, lat_ss, config.aprs_table, lon_dd, lon_mm, lon_ss, config.aprs_symbol);
    }
    else
    {
      sprintf(loc, "!%02d%02d.%02dN%c%03d%02d.%02dE%c", lat_dd, lat_mm, lat_ss, config.aprs_table, lon_dd, lon_mm, lon_ss, config.aprs_symbol);
    ...
    if (config.aprs_ssid == 0)
      sprintf(strtmp, "%s>APDRH1", config.aprs_mycall);
    else
      sprintf(strtmp, "%s-%d>APDRH1", config.aprs_mycall, config.aprs_ssid);
...
// setSyncInterval(600);
  ...
  void taskAPRS(void *pvParameters)
  {
          ...
          if (AFSKInitAct == true)
    {
      ...
      if (millis() > tickInterval)
      {
        ...
        if (config.mygps && gps.speed.isValid())
        {
          ...
          if (config.sta_smartbeacon)
          {
            if (SB_SPEED < config.sta_lspeed && SB_SPEED_OLD > config.sta_lspeed)
            { // STOPING
              SB_SPEED_OLD = 0;
      ...
      if (EVENT_TX_POSITION == 1)
      {
        ...
        if (config.mygps)
        {
          ...
          rawData = myBeacon(String(config.aprs_path));
        }
        else
        {
          rawData = send_fix_location();
          tx_interval = config.aprs_beacon;
        }
        tx_counter = 0;
        EVENT_TX_POSITION = 0;
  ...
  void taskNetwork(void *pvParameters)
  {
    ...
    wifiMulti.addAP("BOUY", "aprsthnetwork");
