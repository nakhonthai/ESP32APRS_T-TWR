...
#define SA868_OPEN_EDITION 1
 
#ifndef SA868_OPEN_EDITION
...
int16_t SA868_getRSSI()
{
    ...
    if (SA868_waitResponse(data, "\r\n", 1000))
    {
        ...
        return (int16_t)rssi.toInt();
    }
    // timeout or error
    return 0;
}

bool SA868_handshake()
{
    String data;
...
bool SA868_SetGroupInstruction(uint8_t band, float freq_tx, float freq_rx, int tone_rx, uint8_t sql_level, int tone_tx)
{
    String data;
    char str[200];
    sprintf(
        str, 
        "AT+DMOSETGROUP=%01d,%0.4f,%0.4f,%d,%01d,%d,0",
        band, 
        freq_tx, 
        freq_rx, 
        tone_rx, 
...
bool SA868_SetVolume(uint8_t volume)
{
    String data;
